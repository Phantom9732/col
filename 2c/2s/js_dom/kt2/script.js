...
for (let i in cats) {
    ...
    for (let key of Object.keys(cat)) {
        if (key != "img_link") {
            ...
            if (key == "name") {
                row.setAttribute("id", `id${cat["id"]}`);
            }
            //key
            let cell1 = document.createElement("td");
            cell1.style.border = '1px solid black';
            let CellText1 = document.createTextNode(key);
            cell1.appendChild(CellText1)
            row.appendChild(cell1)
            cell1.style.marginTop = "10px";
            //value
            let cell2 = document.createElement("td");
            cell2.style.border = '1px solid black';
            let CellText2 = document.createTextNode(cat[key]);
    ...
    let namerow = t.querySelector(`#id${cat["id"]}`);
    let cell_photo = document.createElement("td");
    cell_photo.style.border = '1px solid black';
    let img = document.createElement("img");
    img.src = cat["img_link"];
    //img.width = '300';
    //img.height = '300';
    img.style.height = '300px';
    cell_photo.rowSpan = 6
    cell_photo.appendChild(img);
    cell_photo.style.textAlign = "center";
    
    //cell_photo.style.width = '300px';
    //cell_photo.style.height = '300px';
    namerow.appendChild(cell_photo);
}

div_root.appendChild(t);